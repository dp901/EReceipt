...
//Code Changes to customise TrNgGrid for SkkynetPortal
//1. Removed table-bordered class from tableCssClass
//2. Added table-striped class
//3. Replaced selectedRowCssClass with "selectedRow", was active
//4. Added parameter enableSorting and if(enableSorting) statement (line 134)
//5. Added parameter enableSorting (line 388)
//6. Added ng-class=\"{'linkPointer':currentGridColumnDef.enableSorting}\" ng-click='toggleSorting(currentGridColumnDef.fieldName, currentGridColumnDef.enableSorting)' to div (line 440)
//7. Added parameter enableSorting (line 461)
var TrNgGrid;
(function (TrNgGrid) {
    ...
    var tableCssClass = "tr-ng-grid table table-hover";
    var cellCssClass = "tr-ng-cell";
    var cellTitleSortCssClass = "";
    var titleCssClass = "tr-ng-title col-lg-9 nopadding";
    var sortCssClass = "tr-ng-sort";
    var filterColumnCssClass = "tr-ng-column-filter";
    var filterInputWrapperCssClass = "col-lg-9 nopadding";
    ...
    var selectedRowCssClass = "selectedRow";
    ...
    var GridController = (function () {
        ...
        GridController.prototype.toggleSorting = function (propertyName, enableSorting) {
            if (enableSorting) {
    ...
    angular.module("trNgGrid", []).directive(tableDirective, [function () {
                ...
                compile: function (templateElement, tAttrs) {
                    ...
                    return {
                        ...
                        pre: function (scope, instanceElement, tAttrs, controller) {
                            ...
                            scope.toggleSorting = function (propertyName, enableSorting) {
                                return controller.toggleSorting(propertyName, enableSorting);
                            };
                            scope.filter = "";
                            ...
                            var columnDefSetup = function () {
                                ...
                                scope.currentGridColumnDef.enableFiltering = scope.enableFiltering;//tAttrs["enableFiltering"] == "true" || (typeof (tAttrs["enableFiltering"]) == "undefined" && scope.gridOptions.enableFiltering);
                        ...
                        post: function (scope, instanceElement, tAttrs, controller) {
                            ...
                            if (scope.currentGridColumnDef) {
                                ...
                                if (instanceElement.text() == "") {
                                    ...
                                    $("<div ng-class=\"{'linkPointer sortingTitle':currentGridColumnDef.enableSorting}\" ng-click='toggleSorting(currentGridColumnDef.fieldName, currentGridColumnDef.enableSorting)'>").addClass(titleCssClass).text(scope.currentGridColumnDef.displayName).appendTo(cellContentsTitleSortElement);
        ...
        function () {
            return {
                ...
                template: function (templateElement, tAttrs) {
                    return "<div ng-click='toggleSorting(currentGridColumnDef.fieldName,currentGridColumnDef.enableSorting)' title='Sort' class='nopadding col-lg-1 " + sortCssClass + "'>" +
                        ...
                        //"</div>" +
                        "<i ng-show='currentGridColumnDef.enableSorting && gridOptions.orderBy==currentGridColumnDef.fieldName' ng-class=\"{'fa fa-caret-down': gridOptions.orderByReverse, 'fa fa-caret-up': !gridOptions.orderByReverse}\"></i>" +
                        "</div>";
        ...
        function () {
            return {
                ...
                require: '^' + tableDirective,
                template: function (templateElement, tAttrs) {
                    return '<span ng-show="gridOptions.enableFiltering" class="pull-left form-group">' + '<input class="form-control" type="text" ng-model="gridOptions.filterBy" placeholder="Filter"/>' + '</span>';
                },
                compile: function (templateElement, tAttrs) {
        ...
        function () {
            var setupScope = function (scope, controller) {
                ...
                scope.pageItemsActive = scope.totalItemsCount > 0 && scope.totalItemsCount > 10; //10 is the smallest page size so do not display the selection if the page items count is less than that
                ...
                scope.switchPageSelection = function ($event, pageSelectionActive, pageItemsActive) {
                    scope.pageSelectionActive = pageSelectionActive;
                    scope.pageItemsActive = pageItemsActive;
                    if ($event) {
                        $event.preventDefault();
                        $event.stopPropagation();
                    }
                    ...
                    return '<span class="pull-right form-group">' +
                        '<ul class="pagination">' +
                            '<li ng-show="pageCanGoBack" >' +
                                '<a href="#" ng-click="navigateToPage($event, 0)" title="First Page"><i class="fa fa-angle-double-left" /></a>' +
                            '</li>' +
                            '<li ng-show="pageCanGoBack" >' + '<a href="#" ng-click="navigateToPage($event, gridOptions.currentPage - 1)" title="Previous Page"><i class="fa fa-angle-left" /></a>' + '</li>' +
                            '<li ng-show="pageItemsActive" style="white-space: nowrap;"><span>pages: ' +
                            '<select class="pagingDropdown" ng-model="gridOptions.pageItems"><option value="10">10</option><option value="20">20</option><option value="50">50</option><option value="100">100</option></select>' +
                            '</span></li>' +
                            '<li ng-show="pageSelectionActive" style="white-space: nowrap;">' + '<span>page: ' +
                                '<select class="pagingDropdown" ng-model="gridOptions.currentPage" ng-options="pageIndex as (pageIndex+1) for pageIndex in pageIndexes"></select></span>' + '</li>' +
                            '<li class="disabled" style="white-space: nowrap;">' + '<span ng-hide="totalItemsCount">No items to display</span>' + '<span ng-show="totalItemsCount" title="Select Page">' + '  {{startItemIndex+1}} - {{endItemIndex+1}} displayed <span>, {{totalItemsCount}} in total</span>' + '</span > ' + '</li>' +
                            '<li ng-show="pageCanGoForward">' + '<a href="#" ng-click="navigateToPage($event, gridOptions.currentPage + 1)" title="Next Page"><i class="fa fa-angle-right" /></a>' + '</li>' +
                            '<li>' + '<li ng-show="pageCanGoForward">' + '<a href="#" ng-show="pageCanGoForward" ng-click="navigateToPage($event, lastPageIndex)" title="Last Page"><i class="fa fa-angle-double-right" /></a>' + '</li>' + '</ul>' + '</span>';
                },
                replace: true,
                link: {
                    pre: function (scope, compiledInstanceElement, tAttrs, controller) {

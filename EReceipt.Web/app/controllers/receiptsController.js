angular
    ...
    .controller('receiptsController', [
        '$scope', '$location', '$window', '$filter', '$timeout', 'receiptsService', 'BASE_PATH', function ($scope, $location, $window, $filter, $timeout, receiptsService, BASE_PATH) {
        $scope.receipts = [];
        $scope.invoices = [];
        $scope.deliveryInvoices = [];
        $scope.multiReceipts = [];
        $scope.printedReceipts = [];
        $scope.newReceipts = [];
        $scope.receipt = null;
        ...
        $scope.getMultipleReceipts = function () {
            ...
            receiptsService.getMultipleReceipts($scope.selectedMonth, $scope.selectedYear).then(function (results) {
                ...
                _.each(results.data, function(it) {
                    if (it.receipt.isPrinted)
                        $scope.printedReceipts.push(it);
                    else 
                        $scope.newReceipts.push(it);
                });

                if ($scope.newReceipts.length > 0) {
                    $scope.canSave = true;
                }
                   
                if ($scope.printedReceipts.length > 0) {
                    $scope.canPrint = true;
                }
